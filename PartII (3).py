...
Extra credit (Cycle detection and processing) not attempted
'''
# Linneus3b.py
...
def process(info) :
    ...
    if result_match_object != None :
        ...
        for j in range(len(items)):
            print(j)
        print("made it to before here")
        supersets = get_isa_list(items[0])
        print(supersets)
        #print("This is the first item in items: " + items[0])
        #print("This is the result of the isa list for item 0: " + supersets[0])
        #nextone = get_isa_list(supersets[0])
        #print(nextone)
        #print("Trying to find the thing above " + supersets[0] + " in the list: " + nextone[0])
        #report_chain(items[0], supersets)
        if supersets != [] :
            first = supersets[i]
            a1 = get_article(items[i]).capitalize()
            a2 = get_article(first)
            justification = answer_why(items[0],first)
            print(a1 + " " + items[i] + " is " + a2 + " " + first + justification)
        subsets = get_includes_list(items[i])
        if subsets != [] :
            first = subsets[i]
            a1 = get_article(items[i]).capitalize()
            a2 = get_article(first)
            justification = answer_why(items[0], first)
            print(a1 + " " + items[i] + " is something more general than " + a2 + " " + first + justification)
        else :
            print("I don't know.")
            i+=1
...
def answer_why(x, y):
    ...
    if x == y:
        ...
        return reason
    if isa_test1(x, y):
        reason = "; you told me that directly."
        return reason
    if isa_test1(y,x):
        a1 = get_article(y)
        a2 = get_article(x)
        reason = "; because " + a1 + " " + y + " is " + a2 + " " + x + "."
        return reason
